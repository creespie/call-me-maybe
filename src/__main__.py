...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to the JSON file with prompts to"
        f" process (default: {DEFAULT_INPUT})",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def write_json_file(path: Path, data: Any) -> None:
    ...
    try:
        ...
        print(
            f"Error: could not write output to {path}: {exc}", file=sys.stderr
            )
        raise SystemExit(1)
...
def log_softmax(logits: NDArray[Any]) -> NDArray[Any]:
    m = logits.max()
    return cast(
        numpy.ndarray,
        logits - m - numpy.log(numpy.sum(numpy.exp(logits - m))))


def score_candidate(model: Any, prompt: str, base_ids: list[int]) -> float:
...
def main() -> None:
    ...
    for p in prompts:
        ...
        current_question = model.encode(
            ask_prompt(p["prompt"], functions_definition, temp_name, generated)
        )
        ...
        collision_group = [
            n for n in names if n[len(base_prefix): len(
                base_prefix) + 1] == next_char
        ]

        if len(collision_group) > 1:
            base_generated = f'{{"name": "{base_prefix}'
            base_prompt = ask_prompt(
                p["prompt"], functions_definition, collision_group,
                base_generated
            )
            ...
            for candidate in collision_group:
                ...
                cand_prompt = ask_prompt(
                    p["prompt"], functions_definition, collision_group,
                    cand_generated
                )
                scores[candidate] = score_candidate(
                    model, cand_prompt, base_ids)
            picked = max(scores, key=lambda name: scores[name])

        call = picked
        temp_name = [call]

        # finds function arguments, continuing the SAME generated <tool_call>
        # sequence
        ...
        for idx, pa in enumerate(param_keys):
            is_number = selected_function["parameters"][pa]["type"] in (
                "number",
                "int",
                "float",
                "digit",
            ...
            for _ in range(50):
                generated = (
                    f'{{"name": "{temp_name[0]}", "arguments": '
                    f'{arguments_str}{p_value}'
                )
                current_question = model.encode(
                    ask_prompt(
                        p["prompt"], functions_definition, [temp_name[0]],
                        generated
                    )
                )
                current_question = normalize_input_ids(current_question)
                logits = numpy.array(
                    model.get_logits_from_input_ids(current_question))
