...
def func_parser(
    ...
    for func in json_input:
        ...
        elif not func["description"] or not isinstance(
                func["description"], str):
            sys.exit(f"Missing value in {func['description']}")
        elif not func["parameters"] or not isinstance(
        ...
        for param in func["parameters"].keys():
            ...
            elif not func["parameters"][param]["type"] or not isinstance(
                func["parameters"][param]["type"], str
            ):
                sys.exit(
                    f"Missing value in {func['parameters'][param]['type']}")
...
def prompt_parser(json_input: list[dict[str, Any]]) -> None:
    for func in json_input:
        if "prompt" not in func.keys():
            sys.exit("Missing prompt")
